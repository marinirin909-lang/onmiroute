...
// A 429 on a proxied opencode account sets that proxy aside across requests, not only for
...
describe("OpencodeExecutor proxy refusal memory", () => {
  ...
  it("a 429 sets that proxy aside and later requests do not start on it", async () => {
  ...
  it("a 429 on an account without a proxy writes nothing and direct accounts stay eligible", async () => {
    const mixed = () =>
      credentials([
  ...
  it("a connection without configured accounts never touches the memory", async () => {
    ...
    // The fast path may retry a 429 internally: every planned answer is a 429.
