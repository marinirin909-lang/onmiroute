...
test("source guard: combo.ts skips provider cooldown for per-model-quota 500", () => {
  const src = fs.readFileSync(
  ...
  assert.ok(
    ...
    "executeTargetAttempt must skip provider cooldown recording for per-model-quota providers on 500"
  );
});
